...
{  if(type === "Home"){
       return AlbumCard.map((item) => {
        return (
               ...
               <h1 className="content__img">{item.depth.map((albom)=>{
         return (
               <button key={albom.id} type="button" className="button_home" 
                onClick={()=>onPopupCallback(albom.id)} 
                id={albom.id}>  
               <img src={albom.url} className="picture_home" alt="Логотип"/> 
               </button>
          )})}
               </h1></div>);
      })
      ...
      return imgurlSearch.map((item) => {
            return (
                    <button key={item.id} type="button" className="content__button"
                     onClick={()=>{onPopupCallback(item.id)}} 
                     id={item.id}> 
                     <img src={item.url} className="picture_search" alt="Логотип"/> 
                     </button>
                   );
                 })
   }
